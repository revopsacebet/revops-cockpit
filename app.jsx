...
function Hero({ metric, variant }) {
      ...
      <div className="hero hero-farol">
        ...
        <div className="hf-body">
          ...
          <div className="hf-stats">
            ...
            {metric.trend != null && (
              <div className="hf-trend" title={metric.trendTitle || "Projeção de fechamento do mês = MTD × (dias do mês ÷ dias decorridos)"}>
  ...
  return (
    <div className="hero">
      ...
      {metric.trend != null && (
        <div className="trend-line" title={metric.trendTitle || "Projeção de fechamento do mês = MTD × (dias do mês ÷ dias decorridos)"}>
...
// ⚠️ M0 FICA DE FORA DO CORTE (2026-08-16, pedido do Luis: "eu preciso que o M0 apareça"). Não é
// exceção de conveniência — o corte não faz sentido nesta coluna:
//   1) O corte existe pra desfazer MISTURA de maturidade: numa janela, umas safras já têm W1 e outras
//      não, e somar tudo puxa o número pra baixo sem avisar. Em M0 dentro do MÊS CORRENTE não há
//      mistura: NENHUMA safra fechou (o mês não acabou), todas estão no mesmo estado. O corte não
//      corrigia viés nenhum — só esvaziava a coluna.
//   2) E esvaziava sempre: numa janela MTD (a mais usada) o bucket de M0 é vazio por definição, então
//      "Total · safras maduras" mostrava "—" em Mult M0/FTD e M0 Ret % o mês inteiro.
//   3) A leitura madura de M0 JÁ EXISTE ao lado, e melhor feita: a coluna **M0 Esp.** projeta o
//      fechamento (realizado × G(runway) ÷ G(idade)). Cortar por maturação duplicava isso e pior.
// Fica valendo: o Total de M0 é o acumulado do mês ATÉ AQUI (mesma leitura do card MTD), com o M0 Esp.
// do lado como projeção de fechamento. ⚠️ Em janela que ATRAVESSA meses (ex. 27/07–16/08) esse total
...
function RetFaixaTable({ data, dateLabel, m0Label, base }) {
  ...
  const cells = (r, heat, mat) => {
    ...
    // M0 é a única coluna do Total que NÃO leva o corte — e a célula tem que dizer isso, senão a linha
    // "Total · safras maduras" promete uma régua que ela não aplica ali. Ver AGG_MAT_HZ.
    const tipMat = (hz) => (mat && hz === 'm0')
      ? 'M0 NÃO entra no corte de maturação: dentro do mês corrente nenhuma safra fechou o M0, então o corte esvaziaria a coluna sem corrigir viés algum (todas estão no mesmo estado). Este é o acumulado do mês ATÉ AQUI — a leitura de fechamento é a coluna M0 Esp., ao lado.'
      : nOf(hz) ? 'Só safras que já fecharam este horizonte (' + nOf(hz) + '). As imaturas ficam de fora do cálculo.' : undefined;
    return [
...
const CENARIOS = [
  ...
  // o trio final é Forecast / Investidor / Interno, batendo com as abas Projection_Revenue[_INTERNO|_investidor]).
  ...
  { id: 'conserv', label: 'Investidor',   color: '#F0997B' },
...
// CURVA ROLLING de setembro/26 (mesmo estudo, nó `D.set.roll`) — depósito acumulado ÷ FTD$ por IDADE
// da safra, sem truncar no fim do mês (D0..D60; termina em 9,01x).
// ⚠️ ELA NÃO É A META — a meta continua sendo o M0 CALENDÁRIO (4,66x, a regra do Luis de 13/09 segue
// valendo). A rolling entra só como a FORMA da curva dentro do mês, e é a única que pode fazer esse
// papel: a base-mês já é um agregado de mês (cada ponto dela já embute o truncamento do calendário),
// então ponderá-la por idade de safra desconta DUAS VEZES.
// A prova de que as duas réguas falam do mesmo alvo está no próprio denominador: ponderando a ROLLING
// pelo FTD$ de todos os dias do mês, nas idades que cada safra terá em 30/09, dá **4,660x** — o M0 do
// mês, na vírgula. Com a base-mês o mesmo cálculo dá 3,918x, ou seja, 16% abaixo do alvo que ela
// mesma declara: era esse buraco que aparecia como "orçado MTD" baixo demais nos dois cards.
// Validação em agosto FECHADO (FTD$ reais): rolling ponderada projetava 3,948x no d31 contra 3,929x
// realizado (+0,5%); a base-mês ponderada dava 3,526x (−10%).
const FAROL_CURVA_M0_ROLL = [
  1.69072, 2.18727, 2.52012, 2.82159, 3.07129, 3.31339,
  3.52128, 3.72585, 3.90984, 4.09367, 4.24150, 4.39786,
  4.54842, 4.68872, 4.83945, 4.97338, 5.08920, 5.20290,
  5.31540, 5.42664, 5.54756, 5.67045, 5.78352, 5.89509,
  5.98494, 6.08138, 6.17633, 6.26884, 6.36243, 6.45093,
  6.54349,
];
// Média da curva ponderada pelo FTD$ do plano, avaliada na IDADE de cada safra no dia do `corte`.
// É o "nível de multiplicador que este conjunto de safras deveria ter mostrado até o corte".
function farolM0Proj_(dias, corte, curva) {
  const cum = (i) => curva[Math.min(Math.max(i, 0), curva.length - 1)];
  const t = Date.parse(corte);
  if (!isFinite(t) || !Array.isArray(dias) || !dias.length) return null;
  let num = 0, den = 0;
  dias.forEach((r) => {
    const w = +r.ftd || 0, ts = Date.parse(r.d);
    if (!(w > 0) || !isFinite(ts) || ts > t) return;
    num += w * cum(Math.round((t - ts) / 864e5));
    den += w;
  });
  return (den > 0) ? (num / den) : null;
}
// Fator ∈ (0,1] = quanto do alvo de FECHAMENTO já é exigível no corte da janela.
//     fator = proj(rolling, safras da janela, hoje) ÷ proj(rolling, safras do mês, fim do mês)
// Numerador e denominador saem da MESMA curva e do MESMO peso (FTD$ diário do plano), então o que
// sobra é puramente o efeito do mix de idades — e o denominador reproduz o alvo do mês (4,660x), que
// é o que amarra o fator na meta de calendário.
// `dias`/`diasMes` = [{d:'YYYY-MM-DD', ftd:Number}] (payload.bp.dias e payload.bp.month.dias).
// Sem a série do mês (backend antigo) cai na régua anterior — degradar pro comportamento de ontem é
// melhor do que sumir com a linha de Orçado.
function farolM0Maturacao_(dias, diasMes) {
  if (!Array.isArray(dias) || !dias.length) return null;
  const corte = dias[dias.length - 1].d;
  if (Array.isArray(diasMes) && diasMes.length) {
    const fimMes = diasMes[diasMes.length - 1].d;
    const num = farolM0Proj_(dias, corte, FAROL_CURVA_M0_ROLL);
    const den = farolM0Proj_(diasMes, fimMes, FAROL_CURVA_M0_ROLL);
    if (num > 0 && den > 0) {
      const fator = num / den;
      return (fator > 0 && fator <= 1.0001) ? Math.min(fator, 1) : null;
    }
  }
  const media = farolM0Proj_(dias, corte, FAROL_CURVA_M0_BM);   // fallback: régua antiga (base-mês)
  if (!(media > 0)) return null;
  const fator = media / FAROL_CURVA_M0_BM[FAROL_CURVA_M0_BM.length - 1];
  return (fator > 0 && fator <= 1.0001) ? Math.min(fator, 1) : null;
}
...
// ⚠️⚠️ 2026-09-07 — CORRIGIDO 2×: primeiro tirei do agosto preso (commit 3dfd9d8 de 02/09 tinha
// atualizado MDD_BP_FTD_MULT/MDD_CURVA pra setembro mas esqueceu esta cópia separada), DEPOIS o Luis
// pegou que a régua ainda estava ERRADA — D14 (4,84x) saía MAIOR que M0 (4,66x), "impossível" na
// leitura dele: "n faz sentido o d14 ser maior que o M0... é calendário essa porra".
// ELE TINHA RAZÃO: esta aba mostra o REALIZADO em régua de CALENDÁRIO (truncado no fim do mês do FTD —
// ver RET_CAL_TIP/[[revops-d14-cohort-vs-calendario-open]], decisão de 26/08), mas eu tinha posto a
// META em régua CORRIDA (`D.set.roll`, sem teto de mês) — a MESMA fonte de MDD_BP_FTD_MULT, que é
// certa PRA LÁ porque a Métricas do dia a dia é a única aba que continua corrida de propósito. Aqui não.
// FIX: trocar `D.set.roll` por `D.set.bm` ("base mês" — a curva do PRÓPRIO estudo já truncada no fim do
// mês). Prova de que resolve: `D.set.bm[30] === D.set.alvo` EXATAMENTE (4,66) — por construção, 30 dias
// corridos cabem inteiros em qualquer mês, então a régua de calendário faz D30 convergir pro M0. E
// bm[14]=4,205 < 4,66 — a invariante D14 ≤ M0 passa a valer por construção, não por sorte. Essa era a
// "ressalva aberta" da decisão de 26/08 ("as metas por coluna vêm do estudo em janela corrida —
// refazer as metas na régua nova é trabalho pendente"). Google/Meta continuam em agosto de propósito
// (o estudo de set só calibrou curva pro Geral).
const PIR_META = {
  ftd: {
    Geral:  { d0: 1.69072, d1: 2.17072, d3: 2.75270, d4: 2.96911, w1: 3.49401, w2: 4.20543, d30: 4.6600, m0: 4.6600 },
  ...
  d0: {
    // idem — ÷ d0 da curva `bm`, não `roll`. w2(D14) 2,4874 < d30/m0 2,7562: mesma invariante, base D0.
    Geral:  { d1: 1.2839, d3: 1.6281, d4: 1.7561, w1: 2.0666, w2: 2.4874, d30: 2.7562, m0: 2.7562 },
...
// ============================================================
// META DE UMA SELEÇÃO DE VÁRIOS CANAIS (pedido do Luis 16/08: "preciso que a ferramenta faça o cálculo
// de qual seria a meta sem um ou outro canal no filtro")
// ============================================================
// Antes, 2+ canais devolvia null e a régua sumia da tela inteira — justo no uso mais interessante
// ("quanto eu teria que entregar se tirasse o Google?").
//
// A conta NÃO é média simples: a métrica da tela é Σacumulado ÷ Σbase, então a meta do CONJUNTO é a
// média das metas ponderada pela BASE de cada canal — FTD$ ou depósito do D0, seguindo o toggle
// (mesma base do denominador, senão a meta responde a uma pergunta diferente da célula).
//     meta_S[hz] = Σ_{c∈S} meta_c[hz] × base_c  ÷  Σ_{c∈S} base_c
// Se cada canal entregar a meta dele, o agregado dá exatamente isto. É a mesma álgebra do Total.
//
// ⚠️ PESO = MIX REALIZADO NA JANELA, não mix do plano. Ou seja: "dado o mix que você de fato trouxe,
// qual é a régua justa". Muda com o período e com o filtro de faixa, e é isso que se quer — tirar o
// Google do filtro tem que mexer na meta porque muda o mix, não porque muda a definição.
// ⚠️ SIMPLIFICAÇÃO ASSUMIDA: o peso é o da JANELA INTEIRA, igual pra todas as colunas. Com "só safras
// maduras" ligado, a coluna D30 vê um subconjunto de safras que pode ter mix um pouco diferente. É
// efeito de 2ª ordem e o cabeçalho mostra UMA meta por coluna — reponderar por coluna deixaria a régua
// pulando conforme o toggle, o que confunde mais do que corrige.
//
// ⚠️ CANAL SEM META (Kwai) não some da conta em silêncio: ele fica FORA do numerador e do denominador,
// e a cobertura (`_cob`, fração do FTD$ da seleção que tem meta) vai pra tela. Abaixo de 50% de
// cobertura a coluna NÃO mostra meta — uma régua feita com menos da metade da seleção mentiria.
const PIR_MIX_KEYS = ['d0', 'd1', 'd3', 'd4', 'w1', 'w2', 'd30', 'm0'];
const PIR_MIX_MIN_COB = 0.5;
function pirPesos_(rows, selFx, base) {
  const w = {};
  (rows || []).forEach((r) => {
    if (selFx && !selFx(r.faixa)) return;
    const v = (base === 'd0') ? (r.d0 || 0) : (r.ftd || 0);
    if (!(v > 0)) return;
    w[r.canal] = (w[r.canal] || 0) + v;
  });
  return w;
}
function pirBpMix_(sel, T, pesos) {
  const num = {}, den = {};
  let tot = 0;
  sel.forEach((ch) => {
    const p = (pesos && pesos[ch]) || 0;
    tot += p;
    const cur = T[ch];
    if (!cur || !(p > 0)) return;
    // ⚠️ Acumular numerador E denominador POR CHAVE. Um denominador único quebraria a média sempre que
    // um canal da seleção não tivesse aquele horizonte: o peso dele entraria embaixo sem entrar em cima.
    PIR_MIX_KEYS.forEach((k) => {
      if (cur[k] == null) return;
      num[k] = (num[k] || 0) + cur[k] * p;
      den[k] = (den[k] || 0) + p;
    });
  });
  if (!(tot > 0)) return null;
  const out = { _mix: true, _cob: {}, _cobMin: 1 };
  let algum = false;
  PIR_MIX_KEYS.forEach((k) => {
    if (!(den[k] > 0)) return;
    const cob = den[k] / tot;
    out._cob[k] = cob;
    if (cob < PIR_MIX_MIN_COB) return;   // meta feita com menos da metade da seleção não vai pra tela
    out[k] = num[k] / den[k];
    if (cob < out._cobMin) out._cobMin = cob;
    algum = true;
  });
  return algum ? out : null;
}
function pirBpScope_(chFilter, base, mk, pesos) {
...
function TabPiramideCoorte({ retencaoFaixa, chFilter, meta, retFaixaLive }) {
  ...
  // tela. Sem isso, uma meta derivada de premissa fica visualmente idêntica a uma declarada — e é aí
  // que premissa vira "fato".
  const metaMix = !!(bpScope && bpScope._mix);
  const metaDeriv = pirDerivada_(chFilter) || metaMix;
  const chLbl = chLabel_(chFilter);
  const faixaLbl = faixaSel.length === 0 ? 'todas as faixas' : (faixaSel.length <= 2 ? faixaSel.map(fxLabel_).join(' + ') : faixaSel.length + ' faixas');
    ...
    <React.Fragment>
      ...
      <div className="support pir-nored">
        ...
        <div className="pir-legend">
          ...
          {metaMix && (
            <span style={{ color: 'var(--accent-yellow)' }}
                  title={'Média das metas dos canais selecionados, pesada pelo ' + (base === 'ftd' ? 'FTD$' : 'depósito do D0') + ' que cada um trouxe nesta janela (a mesma base do denominador da célula). Se cada canal entregar a meta dele, o agregado dá exatamente este número.'}>
              † meta ponderada pelo mix de {chList_(chFilter).length} canais nesta janela
              {bpScope._cobMin < 0.999 ? ' · cobertura ' + fmtPct(bpScope._cobMin, 1) : ''}
            </span>
          )}
          {!bpScope && chList_(chFilter).length > 1 && (
            <span style={{ color: 'var(--text-muted)' }}
                  title="Nenhum canal da seleção tem meta declarada ou derivada — não há de onde ponderar. Acontece, por exemplo, selecionando só o Kwai junto de outro canal sem régua.">
              sem meta para esta combinação de canais
            </span>
          )}
          {!bpScope && chList_(chFilter).length === 1 && (
            <span style={{ color: 'var(--text-muted)' }}
                    ...
                    <th key={c.key} className={(sepCls[i] ? sepCls[i] + ' ' : '') + 'pir-th-ord' + (ordKey === c.key ? ' pir-th-on' : '')}
                      ...
                      {mt != null && (
                        <span className={'pir-meta-tag' + (metaDeriv ? ' pir-meta-deriv' : '')}
                              title={metaMix
                                ? ('Meta PONDERADA da seleção: média das metas dos canais escolhidos, pesada pelo '
                                   + (base === 'ftd' ? 'FTD$' : 'depósito do D0') + ' que cada um trouxe nesta janela — a mesma base do denominador da célula. '
                                   + 'Tirar ou pôr um canal no filtro muda a meta porque muda o MIX.'
                                   + (bpScope._cob && bpScope._cob[c.bp] != null && bpScope._cob[c.bp] < 0.999
                                      ? ' ⚠ Cobertura ' + fmtPct(bpScope._cob[c.bp], 1) + ': o resto da seleção não tem meta declarada e ficou de fora dos dois lados da conta.'
                                      : ''))
                                : metaDeriv
...
// ============================================================
// RÉGUA CORRIDA — D60/D30 · D90/D60 · D120/D90 · D120+ (2026-08-25, ampliada em 02/09)
// ============================================================
...
//   CORRIDA           — D60/D30, D90/D60, D120/D90, D120+ : blocos de 30 dias contados do DIA DO FTD de
//                       cada conta. A linha volta a ser a SAFRA (é propriedade da coorte, como o
//                       multiplicador). O rótulo é o dia de INÍCIO do bloco (ver ESC_COLS_RET_DIA).
//
...
// entra "por baixo" puxando a média). Ver escJanFechada_. No pool D120+ o corte é por BLOCO: o bloco
// que não fechou sai dos DOIS lados da razão, nunca só de cima.
//
...
// ============================================================
// ⚠️ O RÓTULO É O DIA FINAL DO BLOCO (rótulo por dia de INÍCIO ficou só entre 02/09 e 03/09/2026 — o
// Luis pediu de volta pro dia final, que é como ele lê M1/M0: "o mês +1 obrigatoriamente tem que
// iniciar depois de 30 dias", i.e. o rótulo é QUANDO o bloco fecha, não quando começa)
// ============================================================
// bloco 0 = dias 0–29 → "D30" (fecha aos 30 dias) · bloco 1 = 30–59 → "D60" · bloco 2 = 60–89 → "D90"
// · bloco 3 = 90–119 → "D120". A coluna `D60/D30` é bloco1÷bloco0 — o análogo direto de M1/M0
// (dep[1]÷dep[0]): o bloco0 é o 1º período de 30 dias (como o M0), o bloco1 é o 2º (como o M1).
const ESC_COLS_RET_DIA = [
  { key: 'w1', lb: 'D60/D30', blk: 'ret', jan: 1,
    tip: 'JANELA CORRIDA: o depositado nos dias 30–59 desde o FTD da própria conta ÷ o depositado nos dias 0–29 (o dia do FTD inclusive). '
       + 'É o análogo direto do M1/M0. A linha é a SAFRA que está sendo medida — diferente da régua mensal, onde a linha é o mês de referência. '
  ...
  { key: 'w2', lb: 'D90/D60', blk: 'ret', jan: 2,
    tip: 'JANELA CORRIDA: dias 60–89 desde o FTD ÷ dias 30–59 — o análogo do M2/M1. É o 2º degrau da mesma escada: mede se quem sobreviveu '
       + 'ao 1º bloco continua depositando no mesmo ritmo. Perto de 100% diz que a base que ficou é estável; o buraco está no degrau anterior.' },
  { key: 'w3', lb: 'D120/D90', blk: 'ret', jan: 3,
    tip: 'JANELA CORRIDA: dias 90–119 desde o FTD ÷ dias 60–89 — o 3º degrau, o análogo do M3/M2 que a régua mensal não tem. '
       + '⚠️ Só conta a safra cujo ÚLTIMO FTD já viveu 119 dias, então com "só safras maduras" ligado as ~4 safras mais novas ficam vazias.' },
  // ⚠️ ANÁLOGO DO M3+, E É UM DEGRAU — NÃO UM ACUMULADO. A conta é Σ bloco[k] ÷ Σ bloco[k−1] para todo
  // k >= 3, exatamente a álgebra do M3+ mensal (Σ dep(R) das safras de idade ≥3 ÷ Σ dep(R−1) das mesmas
  // safras), só trocando o eixo de calendário pelo dia do FTD de cada conta.
  // A OUTRA leitura possível de "D120+ / (D120+ + D60)" — acumular tudo de k>=3 em cima e dividir pelo
  // bloco 2 mais esse mesmo acumulado — foi DESCARTADA de propósito: essa razão cresce sozinha com a
  // IDADE da safra (mais blocos somados no numerador ⇒ tende a 100% na safra velha), então a coluna
  // rankearia idade, não retenção. Se a leitura pedida for a outra, é uma linha aqui e o rótulo muda.
  { key: 'wp', lb: 'D120+', blk: 'ret', jan: 'p',
    tip: 'JANELA CORRIDA, análogo do M3+: Σ do bloco k ÷ Σ do bloco k−1, para TODO bloco de 30 dias a partir do 4º (dias 90+). '
       + 'É a mesma álgebra do M3+ mensal (Σ dep(R) das safras de idade ≥3 ÷ Σ dep(R−1) das mesmas safras), com o eixo no dia do FTD. '
       + '⚠️ É DEGRAU, não acumulado: NÃO é "quanto do dinheiro pós-D60 acontece depois do D90" — essa razão subiria só com a idade da '
       + 'safra e a coluna rankearia idade. Cada bloco entra dividido pelo bloco imediatamente anterior, então a safra velha não ganha de graça. '
       + '⚠️ Não tem coluna de Reativados: reativado é definido em 2+ MESES secos e não tem tradução dentro de uma janela de 30 dias.' },
];
// ⚠️ AS 4 COLUNAS VIVEM NO TOGGLE, não ao lado das mensais (decisão do Luis, 02/09). A 1ª versão deste
// pedido colocava a grade corrida como um bloco EXTRA à direita das mensais; ele preferiu o toggle — a
...
// O toggle TROCA o bloco de retenção inteiro (e o da Lottu junto), nunca soma colunas:
//   · régua MENSAL  → M1/M0 · M2/M1 · M3+ · Reativados            (+ lottu M1/M0 · M2/M1 · M3+)
//   · régua CORRIDA → D60/D30 · D90/D60 · D120/D90 · D120+           (+ lottu D60/D30 · D90/D60)
// As duas responderiam à mesma pergunta em eixos de tempo diferentes lado a lado, mas a tabela já tem
// 16 colunas: somar 4 permanentes empurra o multiplicador pra fora da tela. Fica no toggle.
...
// Blocos que são RETENÇÃO (%, sufixo "jog", tooltip de razão): o nosso ('ret' — mensal ou corrido,
// conforme a régua) e o da Lottu. Existe como função porque são 4 sítios de leitura e um deles
// esquecido significa multiplicador formatado como "1,8x" numa coluna que é 18,5%.
const escEhRet_ = (c) => (c.blk === 'ret' || c.blk === 'lottu');
...
function escRetDia_(uns, col, diaOk, soMaduras, baseRet) {
  const pool = (col.jan === 'p');
  const k = pool ? 3 : col.jan;
  let num = 0, den = 0, n = 0, nUso = 0, aberta = false;
  (uns || []).forEach((u) => {
    n++;
    const arr = ((baseRet === 'jog') ? u.jdw : u.dw) || [];
    if (!arr.length) return;                       // payload sem a grade corrida
    // ---- POOL D120+ : Σ bloco[j] ÷ Σ bloco[j−1] para todo j >= 3. Bloco a bloco, cada um com o SEU
    // antecessor embaixo — é a mesma álgebra do M3+ mensal e é o que mantém a coluna imune à idade da
    // safra (acumular só o numerador faria a safra velha tender a 100% por ter mais blocos somados).
    // ⚠️ A maturação é por BLOCO, não pela safra inteira: com "só safras maduras" um bloco que ainda não
    // fechou 30 dias fica fora dos DOIS lados da razão, nunca só de cima.
    if (pool) {
      let usou = false;
      for (let j = 3; j < arr.length; j++) {
        const nv2 = arr[j] || 0, dv2 = arr[j - 1] || 0;
        if (!nv2 && !dv2) continue;
        const fj = escJanFechada_(u.safra, j, diaOk);
        if (soMaduras && !fj) continue;
        num += nv2; den += dv2; usou = true;
        if (!fj) aberta = true;
      }
      if (usou) nUso++;
      return;
...
function TabEscadaMensal({ chFilter, meta, metric, lottuEscada }) {
  ...
  // ============================================================
  // ⚠️ GRADE CORRIDA TRUNCADA NO 3º BLOCO — o caso do meio, e o mais perigoso dos três
  // ============================================================
  // Até 02/09/2026 o backend cortava o join da janela corrida em 90 dias (`INTERVAL 90 DAY`), então
  // `dw` vinha com no máximo 3 blocos. Nesse payload o D120/D90 NÃO fica vazio: ele sai `0 ÷ dw[2]` =
  // **0,0%** — porque "bloco ausente do vetor" é, no contrato do payload, "não depositou naquele bloco"
  // (é o que faz `ago D60/D30 = 0/700` ser dado, e não célula vazia). Ou seja: com o backend velho a tela
  // mostraria "a retenção do 3º degrau é ZERO", que é a leitura errada mais cara possível aqui.
  // Por isso, detectado o truncamento, as colunas do 4º bloco em diante SAEM DA TABELA e um aviso diz
  ...
  const celula = (l, c, sep) => {
      ...
      return (
        <td key={c.key} className={sep || undefined}>
          <span className="pir-v pir-excl" title={cel.semDw
            ...
            : (c.jan === 'p')
            ? ('Nenhuma safra desta linha chegou ao 4º bloco de 30 dias (dias 90+) com a janela fechada. '
               + 'O D120+ só começa a existir quando a safra passa dos 120 dias de vida — '
    ...
    const nS = (c.jan === 'p')
      ? (' · janela CORRIDA, pool D120+: Σ de cada bloco de 30 dias a partir do 4º (dias 90+) ÷ Σ do bloco anterior de cada um'
         + ((porSafra && !l._tot) ? ' · mede a safra DESTA linha' : ' · ' + cel.nUso + ' de ' + cel.n + ' safras entraram'))
      : (c.jan != null)
    ...
    const dica = (cel.aberta
      ? (c.jan === 'p'
         ? 'PARCIAL: algum dos blocos somados aqui ainda não fechou os 30 dias'
    ...
    <React.Fragment>
        ...
        <div>
          ...
          <div className="subtitle">
            Grão de MÊS, e <strong>duas leituras na mesma linha</strong>: {met === 'ggr'
              ...
              ? <React.Fragment>está na régua <strong>corrida</strong>: blocos de 30 dias desde o FTD de cada conta (D60/D30, D90/D60, D120/D90 e o pool D120+) — mesma safra da linha, sem o efeito de calendário</React.Fragment>
      ...
      <div className="slicer-group slicer-ruler">
        ...
        <div className="slicer-presets" style={{ marginLeft: 6 }}>
          <button className={`preset-btn ${!porDia ? 'active' : ''}`} onClick={() => setRegua('mes')}
                  title={'MENSAL (M1/M0, M2/M1, M3+, Reativados) — mês-CALENDÁRIO, a régua do Farol e do BP (as metas 75/72/88 falam desta). '
          ...
          <button className={`preset-btn ${porDia ? 'active' : ''}`} onClick={() => setRegua('dia')}
                  title={'CORRIDA (D60/D30, D90/D60, D120/D90, D120+) — blocos de 30 dias contados do DIA DO FTD de cada conta, não do começo do mês. '
                       + 'O rótulo é o dia de INÍCIO do bloco: D0 = dias 0–29, D30 = 30–59, D60 = 60–89, D90 = 90–119. '
                       + 'A linha volta a ser a SAFRA (é propriedade da coorte, como o multiplicador). '
                       + 'O D120+ é o análogo do M3+: Σ de cada bloco a partir do 4º ÷ Σ do bloco anterior de cada um. '
                       ...
                       + '⚠️ Não tem coluna de Reativados: "reativado" é definido em 2+ MESES secos e não tem tradução numa janela de 30 dias.'}>corrida (30d)</button>
        </div>
        ...
        <label className="pir-tgl" style={{ marginLeft: '12px' }}
               title={'Tira do CÁLCULO todo mês que ainda não fechou. Vem LIGADO porque o mês corrente é o M0 da safra deste mês e o M1 da safra do mês passado — desligado, a diagonal mais nova aparece cortada no dia de hoje e "a retenção caiu" vira artefato de calendário. Desligado, o parcial aparece hachurado e é sempre um PISO.'
                    ...
                    + (porDia ? ' ⚠️ NA RÉGUA CORRIDA o mesmo toggle vale em DIAS: a coluna D60/D30 de uma safra só entra quando o ÚLTIMO FTD daquele mês já viveu 59 dias — 89 no D90/D60, 119 no D120/D90. Por isso as safras mais novas ficam vazias nas colunas corridas mesmo tendo mês fechado. No D120+ o corte é por BLOCO: o bloco que ainda não fechou 30 dias sai dos dois lados da razão.' : '')}>
          <input type="checkbox" checked={!!soMaduras} onChange={(e) => setSoMaduras(e.target.checked)} />
      ...
      <div className="support">
        ...
        <div className="pir-legend">
          ...
          {porDia && !temDw && (
            <span style={{ color: 'var(--negative)' }}>
              ⚠ a régua CORRIDA está selecionada mas o backend em produção não manda as janelas de 30 dias (<code>dw</code>) — as colunas D60/D30, D90/D60, D120/D90 e D120+ ficam vazias até o deploy do <code>only=escada</code> propagar.
            </span>
          )}
          {porDia && dwTrunc && (
            <span style={{ color: 'var(--negative)' }}>
              ⚠ o backend em produção ainda corta a janela corrida em <strong>90 dias</strong> — as colunas <code>D120/D90</code> e
              {' '}<code>D120+</code> ficam vazias (e não é falta de safra madura) até o deploy do <code>only=escada</code> propagar.
              {' '}<code>D60/D30</code> e <code>D90/D60</code> seguem corretas.
            </span>
          )}
        ...
        {/* ⚠️ TUDO O QUE É DIDÁTICA VIVE FECHADO. Eram 12 avisos empilhados acima da tabela, 4 linhas de
          ...
          {porDia && (
            <span title={'A régua corrida TEM o análogo do M3+ (a coluna D120+ = Σ de cada bloco a partir do 4º ÷ Σ do bloco anterior de cada '
                       + 'um, a mesma álgebra do M3+ mensal). O que ela não tem é a coluna de REATIVADOS: "reativado" é definido por 2+ MESES '
                       + 'secos e não tem tradução dentro de uma janela de 30 dias. Para a decomposição ficou/voltou, troque para a régua mensal.'}>
              tem <strong>D120+</strong> (o análogo do M3+), mas <strong>não tem Reativados</strong> — reativação é definida em meses
            </span>
          )}
          {!porDia && (
          <span style={{ color: 'var(--accent-yellow)' }}
        ...
        <details className="ch-note">
          ...
          <div style={{ marginTop: 6 }}>
            ...
            blocos de <strong>30 dias desde o dia do FTD de cada conta</strong>, e o <strong>rótulo é o dia de INÍCIO do bloco</strong>:
            <code> D60/D30</code> = dias 30–59 ÷ dias 0–29, <code>D90/D60</code> = dias 60–89 ÷ dias 30–59,
            <code> D120/D90</code> = dias 90–119 ÷ dias 60–89 (bloco ÷ bloco, igual ao M1/M0 que é <code>dep[1]÷dep[0]</code>;
            se a leitura que você quer é a razão dos ACUMULADOS, é 1 + este número). E o <code>D120+</code> é o
            <strong> análogo do M3+</strong>: <code>Σ bloco[k] ÷ Σ bloco[k−1]</code> para todo <code>k ≥ 3</code> — a mesma álgebra do
            M3+ mensal, com o eixo no dia do FTD. ⚠️ É <strong>degrau, não acumulado</strong>: “quanto do dinheiro pós-D60 acontece
            depois do D90” subiria só com a IDADE da safra e a coluna rankearia idade, não retenção.
            ...
            o mesmo eixo) e a maturação passa a ser em <strong>dias</strong> — a coluna <code>D60/D30</code> só conta a safra
            cujo <em>último</em> FTD já viveu 59 dias, o <code>D90/D60</code> 89, o <code>D120/D90</code> 119, e no <code>D120+</code> o
            corte é por <strong>bloco</strong> (o bloco que não fechou sai dos dois lados da razão). Nesta régua
            <strong> não existe a coluna de Reativados</strong>: reativação é definida em 2+ meses secos e não tem tradução
            numa janela de 30 dias.
            ...
            {temLottu && (<span>
              ...
              D60/D30 e D90/D60, dos mesmos blocos de 30 dias contados do FTD de cada conta dela (a grade dela para no 3º
              bloco, então <code>D120/D90</code> e <code>D120+</code> não têm par do lado da Lottu até o
              <code> tools/lottu-escada/build.js</code> rodar de novo sem o teto de 90 dias). Saem do
...
function buildFarolMetrics_(M, comp, channels, ggrChannels, bp, filter, ggrSafra) {
  ...
  const matM0 = farolM0Maturacao_((bp && bp.dias) || null, (bp && bp.month && bp.month.dias) || null);
  const roasDepM0Mes = roasDepM0Bp, multM0Mes = multM0Bp;   // alvo de FECHAMENTO, antes da maturação
  if (matM0 != null) {
    if (roasDepM0Bp != null) roasDepM0Bp *= matM0;
    if (multM0Bp != null) multM0Bp *= matM0;
  }
  // PROJEÇÃO DE FECHAMENTO dos dois cards de razão (18/09) — a pergunta que o Luis faz olhando o card
  // é "nesse ritmo, fecho a meta do mês?". Pros cards de VOLUME o `dress` já projeta por regra de três
  // de dias; razão de M0 não pode ser projetada assim (o denominador não cresce junto). A projeção
  // certa é dividir o realizado pelo MESMO fator de maturação: se as safras da janela já deveriam ter
  // mostrado `fator` do nível de fechamento, o fechamento implícito no ritmo de hoje é act ÷ fator.
  const actRoasM0 = div(dm0.act, inv.act), actMultM0 = div(dm0.act, fa.act);
  const projM0 = (v) => (matM0 != null && matM0 > 0 && v != null && isFinite(v)) ? v / matM0 : null;
  const m0TrendTitle = 'Projeção de fechamento pela curva de maturação: realizado até aqui ÷ '
    + (matM0 != null ? (matM0 * 100).toFixed(0) + '%' : 'fator') + ' (a fatia do alvo do mês que estas '
    + 'safras já deveriam ter entregue). Não é regra de três de dias — o denominador da razão não cresce junto.';
  return {
    ...
    roasDepM0:   Object.assign(mk('ROAS Dep M0', 'multiple', actRoasM0, roasDepM0Bp, div(dm0.m1, inv.m1)),
      { matM0: matM0, trend: projM0(actRoasM0), trendTitle: m0TrendTitle,
        ...
        + (roasDepM0Mes != null ? ', que é ' + fmtMultiple(roasDepM0Mes) : '') + '. '
