...
function buildFarolGroups_(MM, f, range, useYtd, sparkByKey, scenLabel) {
  ...
  // ROAS GGR M0 — meta DERIVADA do plano, não constante: ROAS Dep M0 do plano × GGR/Dep do plano.
  // (quanto depósito M0 cada real de mídia compra) × (quanto daquele depósito vira receita). Assim ela
  // se move junto com o toggle de cenário, em vez de ficar num número solto. Ago/26 Forecast: 1,70 ×
  // 15,0% = 25,5% — bate com os 23,72% que a aba Multiplicadores já usava como meta editável.
  // ⚠️ CAVEAT: o GGR/Dep do plano é BLEND da casa (todas as safras); aqui ele é aplicado a um numerador
  // só de M0. Sem meta de margem POR SAFRA no plano, é a melhor âncora disponível — e é transparente.
  // Herda scenBp/fcBp dos dois componentes p/ o rótulo dizer "Forecast" só quando de fato é o do cenário.
  const rdm = f.roasDepM0 || {}, gpd = MM.ggrPerDep || {};
  const roasGgrM0Bp = (rdm.bp != null && gpd.bp != null) ? rdm.bp * gpd.bp : null;
  const roasGgrM0Card = dressPlain(f.roasGgrM0 ? { ...f.roasGgrM0,
    bp: roasGgrM0Bp,
    pctBp: (roasGgrM0Bp && f.roasGgrM0.act != null) ? f.roasGgrM0.act / roasGgrM0Bp : null,
    scenBp: !!(rdm.scenBp || gpd.scenBp), fcBp: !!(rdm.fcBp || gpd.fcBp),
  } : null);
...
function buildFarolMetrics_(M, comp, channels, ggrChannels, bp, filter, ggrSafra) {
  ...
  return {
    ...
    // Meta fica null aqui: ela é DERIVADA do plano (ROAS Dep M0 × GGR/Dep) e só dá pra montar depois que o
    // cenário foi aplicado — ver roasGgrM0Card em buildFarolGroups_.
    roasGgrM0:   mk('ROAS GGR M0', 'multiple', div(ggrM0Sum, inv.act), null, div(ggrM0SumL, inv.m1)),
    ...safraMargem,
